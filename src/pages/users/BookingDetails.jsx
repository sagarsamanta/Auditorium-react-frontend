...
const BookingDetails = (props) => {
  ...
  const getShowsDetails = () => {
    Axios("GET", `/user/get-booking-details/${bookingId}`, null, {
      authRequest: true,
      token: token,
    })
      .then((response) => {
  ...
  const cancelTicketModal = () => {
    ...
    Axios("DELETE", `/show/${bookingDetails?._id}`, null, {
      authRequest: true,
      token: token,
    })
      .then((response) => {
  ...
  return (
    <div className="text-sm mx-auto container px-2 sm:px-6 lg:px-8 py-4 space-y-4 min-h-screen lg:mx-auto mt-3 pb-14 relative">
        ...
        <div className={`bg-gray-800 text-skin-inverted rounded-lg p-4`}>
          <div className={`block py-4 text-skin-inverted transition-all`}>
            <div className="flex flex-col md:flex-row items-start gap-4">
              <div
                className={`w-full md:w-1/3 h-[360px] max-h-[360px] relative overflow-hidden rounded-md shadow-lg border border-white/5`}
              >
                <img
              ...
              <div className="flex w-full flex-col justify-end">
                ...
                <div className="mt-4 space-y-1">
                  ...
                  <p className="line-clamp-3">
                    {bookingDetails?.movie?.description}
                  </p>
                </div>
                <div className="mt-4 space-y-1">
                  <span className="text-lg font-semibold">Realese Date</span>
                  <p className="line-clamp-3">
                    {displayDate(bookingDetails?.movie?.releaseDate)} |{" "}
      ...
      {!error && viewTicket && (
        <TicketModal
          handleDownloadTickets={handleDownloadTickets}
          closeHandler={closeTicketModal}
          isOpen={viewTicket}
          ticket={ticketDeatils}
        />
      )}
    </div>
  );
