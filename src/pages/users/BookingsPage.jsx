...
const BookingsPage = () => {
  ...
  const upCommingShows = bookings?.filter(
    (booking) => !isPastDate(booking?.movie?.releaseDate)
  );
  const previousShows = bookings?.filter((booking) =>
    isPastDate(booking?.movie?.releaseDate)
  );
  ...
  useEffect(() => {
    Axios("GET", `/user/get-all-booked-movie/${user?._id}`, null, {
      authRequest: true,
      token: token,
    })
      .then((response) => {
  ...
  return (
    <div className="text-sm mx-auto container px-2 sm:px-6 lg:px-8 py-4 space-y-4 min-h-screen lg:mx-auto mt-3 pb-14 relative">
      <h1 className="text-xl md:text-2xl lg:text-3xl p-3 text-white rounded-sm font-semibold mb-4">
        Booking History
      </h1>
      {loading ? (
              ...
              <div className="py-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                {upCommingShows?.map((booking) => {
                  return (
                    <div
                      key={booking?._id}
                      className={`p-4 text-skin-inverted transition-all flex flex-col md:flex-row items-center gap-4 bg-gray-700/30 rounded-md ${
                    ...
                    >
                      <div
                        className={`w-full md:w-1/2 h-[300px] min-h-[300px] max-h-[300px] relative overflow-hidden rounded-md shadow-lg border border-white/5`}
                      >
                        <img
                      ...
                      <div className="flex w-full flex-col justify-end">
                        <div className="">
                          <h5 className="font-semibold w-full text-lg uppercase leading-tight">
                            {booking?.movie?.title}
                          </h5>
                        ...
                        <div className="grid grid-cols-2">
                          <div className="mt-2">
                            ...
                            <p className="line-clamp-3">
                              {displayDate(booking?.movie?.releaseDate)} |{" "}
                ...
                {previousShows?.map((booking) => {
                  return (
                    <>
                      <Link
                        to={`/user/bookings/${booking?._id}`}
                        className={`booking rounded block bg-gray-700/30 py-4 px-2 text-gray-100 group transition-all`}
                      >
                        <div className="flex items-start gap-4">
                          <div className="w-40 h-full overflow-hidden rounded shadow-lg border border-white/5">
                            <img
                              src={lowResImageUrl(booking?.movie?.poster)}
                              alt="not-found"
                              className="transition delay-150 grayscale group-hover:grayscale-0"
                            />
                          </div>
                          <div className="flex w-full flex-col justify-end">
