...
const LiveBookings = () => {
  ...
  const [totalAmountCollectedOnShow, setTotalAmountCollectedOnShow] =
    useState(0);
  ...
  useEffect(() => {
    Axios("GET", `movie/movie-shows-timings`, null, {
      ...
      .then((res) => {
        ...
        setAllShows(
          res.data?.show.filter((show) => show.status === STATUS_ACTIVE)
        );
            ...
            <div className="border p-4 rounded-lg flex flex-col shadow-md">
              <span className="text-lg font-semibold">
                {selectedShow?.totalSeats -
                  (selectedShow?.reservedSeats + selectedShow?.bookedSeats) ||
                  0}
              </span>
