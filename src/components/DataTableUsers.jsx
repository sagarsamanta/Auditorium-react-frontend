...
const DataTableUsers = ({ data, className }) => {
    ...
    const [changePasswordModal, setChangePasswordModal] = useState({ user: {}, newPassword: '', show: false, loading: false, error: '' });
    ...
    const handleNewPasswordInput = (e) => {
        setChangePasswordModal({...changePasswordModal, newPassword: e?.target?.value});
    }
    ...
    const saveNewPassword = () => {
        if (changePasswordModal?.user && changePasswordModal?.newPassword) {
            const newUserPassword = {empId: changePasswordModal.user?.empId, newPassword: changePasswordModal.newPassword};
            setChangePasswordModal({...changePasswordModal, loading: true})
            Axios("PUT", "user/chnage-password", newUserPassword, {authRequest: true, token: token})
            .then((res) => {
                if (res.status === 200) {
            ...
            <>
                ...
                <div className="flex justify-center items-center gap-4">
                    <div className="w-full">
                        ...
                        />
                        <p className={`${changePasswordModal?.error && !changePasswordModal?.newPassword?.length ? 'block' : 'hidden'} text-red-700`}>Enter Password</p>
                    </div>
                    <button
    ...
    return (
        <>
            ...
            <Modal isOpen={changePasswordModal?.show} closeHandler={changePasswordModalHandler} config={chnagePasswordModalConfig} />
        </>
    )
}
