...
export const userProfileImage = (user) => {
  ...
  return (
    <div
      title={user.name}
      className="h-8 w-8 rounded-full capitalize bg-skin-base flex items-center justify-center text-white"
    >
      {initials}
    </div>
  );
...
export const getSeatsForShow = async (movieId, showId, token) => {
  ...
  const res = await fetch(
    `${API_ROOT}/user/get-all-seats-details/${movieId}/${showId}`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export const getAdminAllTypesBookings = async (userId, token) => {
  const res = await fetch(`${API_ROOT}/booking/get-all-bookings`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
    cache: "no-store",
...
export const getAdminUsersBookings = async (userId, token) => {
    const res = await fetch(`${API_ROOT}/booking/get-all-user-bookings`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
      cache: "no-store",
...
export const getAdminOwnBookings = async (userId, token) => {
  const res = await fetch(`${API_ROOT}/booking/get-all-own-bookings`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
    cache: "no-store",
